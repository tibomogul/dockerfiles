...
There is some SSH forwarding magic going on in Docker Desktop vs Docker Engine. In order to accommodate both, we use a script to determine what host-side "file" to use. Add this to your login script (`.bashrc` or `.zshrc`)
```bash
# Check Docker version
DOCKER_VERSION_OUTPUT=$(docker version 2>/dev/null)

# Set DOCKER_SSH_AUTH_SOCK based on Docker version output
if echo "$DOCKER_VERSION_OUTPUT" | grep -q "Server: Docker Desktop"; then
  export DOCKER_SSH_AUTH_SOCK="/run/host-services/ssh-auth.sock"
else
  export DOCKER_SSH_AUTH_SOCK="$SSH_AUTH_SOCK"
fi

# Check for darwin and call ssh-add if found
if echo "$DOCKER_VERSION_OUTPUT" | grep -q "darwin"; then
  ssh-add --apple-load-keychain 2>/dev/null
fi
```

#### Mac
The above also loads ssh keys that has been added to the keychain on the Mac. If you have not added your ssh-keys yet, add them using `ssh-add --apple-use-keychain <your key>`.
Also, add the following to your `~/.ssh/config` file. Replace with your own list of keys.
```
Host *
    UseKeychain yes
    AddKeysToAgent yes
    IdentityFile ~/.ssh/id_rsa
    IdentityFile ~/.ssh/id_ed25519_llh
    IdentityFile ~/.ssh/id_ed25519_vp
```
...
### Check and run
Run `ssh-add -l`, this should list the keys you added.
...
docker run --rm -it \
  ...
  tibomogul/rbenv_nvm
```
Then in the container, verify you can authenticate, e.g. `ssh -Tv git@github.com`.

In the Mac, if you run into permissions error, that would be the socker reverting to root permissions only, like after a reboot. Inside the container, run `sudo chmod o+rw /run/host-services/ssh-auth.sock` and then you'll be able to authenticate.
...
COPY --chown=${USER_NAME} package.json pnpm-lock.yaml ./
